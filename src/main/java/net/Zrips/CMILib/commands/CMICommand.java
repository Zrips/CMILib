...
public class CMICommand {
    ...
    public static void performCommand(Player player, List<String> commands, CommandType type) {
        for (String command : commands) {
            ...
            if (!event.isCancelled()) {
                if (Version.isFolia())
                    CMIScheduler.runAtEntity(player, () -> player.performCommand(event.getMessage().startsWith("/") ? event.getMessage().substring(1, event.getMessage().length()) : event.getMessage()));
                else
