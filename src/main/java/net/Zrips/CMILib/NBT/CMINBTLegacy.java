...
public class CMINBTLegacy implements CMINBTInterface {
    ...
    public CMINBTLegacy(Object nbtTagCompound) {
        tag = nbtTagCompound;
        type = nmbtType.custom;
    }

    public CMINBTLegacy(ItemStack item) {
        object = item;
        tag = getNbt(item);
        type = nmbtType.item;
    }

    public CMINBTLegacy(Block block) {
        tag = getNbt(block);
        object = block;
        type = nmbtType.block;
    }

    public CMINBTLegacy(Entity entity) {
        tag = getNbt(entity);
        object = entity;
    ...
    private Object updateLegacyTag(Object t) {
        ...
        if (getType().equals(nmbtType.item) && Version.isCurrentEqualOrHigher(Version.v1_20_R4)) {
            t = getFromOptional(new CMINBTLegacy(t).get("components"));
            t = getFromOptional(new CMINBTLegacy(t).get("minecraft:custom_data"));
        }
        return t;
    }
