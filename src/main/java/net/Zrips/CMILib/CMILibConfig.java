...
import net.Zrips.CMILib.Colors.CMIGlobalColor;
...
//        mobNameForItemStack = cfg.get("Heads.MobName", false);
		...
		cfg.addComment("Colors.Global", "Defining global chat colors used in locale file", "This will allow you to modify multiple colors at once");
		for (CMIGlobalColor one : CMIGlobalColor.values()) {
			cfg.addComment("Colors.Global." + one.toString(), "Use " + one.getColorId() + " to use this color. Default: " + CMIChatColor.deColorize(one.getColor()), one.getDescription());
			one.setColor(cfg.get("Colors.Global." + one.toString(), CMIChatColor.deColorize(one.getColor())));
		}
